 ...
 */
var addDOMNodeListenerOfFacebookPlugin = function addDOMNodeListenerOfFacebookPlugin(callback) {
  // check compatibility of MutationObserver
  var MutationObserver = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver;
  if (MutationObserver) {
    // create an observer instance
    var observer = new MutationObserver(function (mutations, observerSelf) {
      mutations.forEach(function (mutation) {
        if (mutation.target.id === 'fb-root' && mutation.addedNodes.length > 0) {
          var addedNodes = mutation.addedNodes;

          addedNodes.forEach(function (element) {
            if (element && element.className && typeof element.className === 'string' && element.className.includes('fb_dialog')) {
              callback();
    ...
    console.warn('MutationObserver and addEventListener are not supported! Callback function of addDOMNodeListenerOfFacebookPlugin would not work.');
  }
};
  ...
  _createClass(MessengerCustomerChat, [{
    ...
    value: function render() {
      ...
      if (fbLoaded && shouldShowDialog !== this.props.shouldShowDialog) {
        addDOMNodeListenerOfFacebookPlugin(function () {
